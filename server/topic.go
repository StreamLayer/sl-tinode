	...
	for {
		...
		case leave := <-t.unreg:
			...
			if t.isInactive() {
				...
				if err := t.replyLeaveUnsub(hub, leave.sess, asUid, leave.pkt.Id, leave.pkt.Timestamp); err != nil {
		...
		case meta := <-t.meta:
			...
			case meta.pkt.Get != nil:
				...
				if meta.pkt.MetaWhat&constMsgMetaDesc != 0 {
					if err := t.replyGetDesc(meta.sess, asUid, meta.pkt.Get.Id, meta.pkt.Timestamp, meta.pkt.Get.Desc); err != nil {
						log.Printf("topic[%s] meta.Get.Desc failed: %s", t.name, err)
					}
				}
				if meta.pkt.MetaWhat&constMsgMetaSub != 0 {
					if err := t.replyGetSub(meta.sess, asUid, authLevel, meta.pkt.Get.Id, meta.pkt.Timestamp, meta.pkt.Get.Sub); err != nil {
						log.Printf("topic[%s] meta.Get.Sub failed: %s", t.name, err)
					}
				}
				if meta.pkt.MetaWhat&constMsgMetaData != 0 {
					if err := t.replyGetData(meta.sess, asUid, meta.pkt.Get.Id, meta.pkt.Timestamp, meta.pkt.Get.Data); err != nil {
						log.Printf("topic[%s] meta.Get.Data failed: %s", t.name, err)
					}
				}
				if meta.pkt.MetaWhat&constMsgMetaDel != 0 {
					if err := t.replyGetDel(meta.sess, asUid, meta.pkt.Get.Id, meta.pkt.Timestamp, meta.pkt.Get.Del); err != nil {
						log.Printf("topic[%s] meta.Get.Del failed: %s", t.name, err)
					}
				}
				if meta.pkt.MetaWhat&constMsgMetaTags != 0 {
					if err := t.replyGetTags(meta.sess, asUid, meta.pkt.Get.Id, meta.pkt.Timestamp); err != nil {
				...
				if meta.pkt.MetaWhat&constMsgMetaCred != 0 {
					...
					if err := t.replyGetCreds(meta.sess, asUid, meta.pkt.Get.Id, meta.pkt.Timestamp); err != nil {
			...
			case meta.pkt.Set != nil:
				// Set request
				if meta.pkt.MetaWhat&constMsgMetaDesc != 0 {
					if err := t.replySetDesc(meta.sess, asUid, meta.pkt.Timestamp, meta.pkt.Set); err == nil {
				...
				if meta.pkt.MetaWhat&constMsgMetaTags != 0 {
					if err := t.replySetTags(meta.sess, asUid, meta.pkt.Timestamp, meta.pkt.Set); err != nil {
						log.Printf("topic[%s] meta.Set.Tags failed: %v", t.name, err)
					}
				}
				if meta.pkt.MetaWhat&constMsgMetaCred != 0 {
					if err := t.replySetCred(meta.sess, asUid, meta.pkt.Timestamp, authLevel, meta.pkt.Set); err != nil {
			...
			case meta.pkt.Del != nil:
				...
				case constMsgDelMsg:
					err = t.replyDelMsg(meta.sess, asUid, meta.pkt.Timestamp, meta.pkt.Del)
				case constMsgDelSub:
					err = t.replyDelSub(hub, meta.sess, asUid, meta.pkt.Timestamp, meta.pkt.Del)
				case constMsgDelTopic:
					err = t.replyDelTopic(hub, meta.sess, asUid, meta.pkt.Timestamp, meta.pkt.Del)
				case constMsgDelCred:
					err = t.replyDelCred(hub, meta.sess, asUid, meta.pkt.Timestamp, authLevel, meta.pkt.Del)
				}
...
func (t *Topic) handleSubscription(h *Hub, join *sessionJoin) error {
	...
	if getWhat&constMsgMetaDesc != 0 {
		...
		if err := t.replyGetDesc(join.sess, asUid, join.pkt.Id, join.pkt.Timestamp, msgsub.Get.Desc); err != nil {
	...
	if getWhat&constMsgMetaSub != 0 {
		...
		if err := t.replyGetSub(join.sess, asUid, authLevel, join.pkt.Id, join.pkt.Timestamp, msgsub.Get.Sub); err != nil {
	...
	if getWhat&constMsgMetaTags != 0 {
		...
		if err := t.replyGetTags(join.sess, asUid, join.pkt.Id, join.pkt.Timestamp); err != nil {
	...
	if getWhat&constMsgMetaCred != 0 {
		// Send get.tags response as a separate {meta} packet
		if err := t.replyGetCreds(join.sess, asUid, join.pkt.Id, join.pkt.Timestamp); err != nil {
	...
	if getWhat&constMsgMetaData != 0 {
		...
		if err := t.replyGetData(join.sess, asUid, join.pkt.Id, join.pkt.Timestamp, msgsub.Get.Data); err != nil {
	...
	if getWhat&constMsgMetaDel != 0 {
		...
		if err := t.replyGetDel(join.sess, asUid, join.pkt.Id, join.pkt.Timestamp, msgsub.Get.Del); err != nil {
...
func (t *Topic) subscriptionReply(h *Hub, join *sessionJoin) error {
	...
	if changed, err = t.thisUserSub(h, join.sess, asUid, asLvl, join.pkt.Id, mode, join.pkt.Timestamp, private); err != nil {
		return err
	}
...
func (t *Topic) thisUserSub(h *Hub, sess *Session, asUid types.Uid, asLvl auth.Level,
	pktID, want string, incomingReqTs time.Time, private interface{}) (bool, error) {
...
func (t *Topic) anotherUserSub(h *Hub, sess *Session, asUid, target types.Uid, incomingReqTs time.Time, set *MsgClientSet) (bool, error) {
...
func (t *Topic) replyGetDesc(sess *Session, asUid types.Uid, id string, incomingReqTs time.Time, opts *MsgGetOpts) error {
	now := types.TimeNow()
...
func (t *Topic) replySetDesc(sess *Session, asUid types.Uid, incomingReqTs time.Time, set *MsgClientSet) error {
	now := types.TimeNow()
...
func (t *Topic) replyGetSub(sess *Session, asUid types.Uid, authLevel auth.Level, id string, incomingReqTs time.Time, req *MsgGetOpts) error {
	now := types.TimeNow()
	...
	if len(subs) > 0 {
		...
		sess.queueOut(NoContentParams(id, t.original(asUid), now, incomingReqTs, map[string]interface{}{"what": "sub"}))
	}
...
func (t *Topic) replySetSub(h *Hub, sess *Session, pkt *ClientComMessage) error {
	...
	if target == asUid {
		...
		changed, err = t.thisUserSub(h, sess, asUid, asLvl, pkt.Id, set.Sub.Mode, pkt.Timestamp, nil)
	} else {
		// Request to approve/change someone's subscription
		changed, err = t.anotherUserSub(h, sess, asUid, target, pkt.Timestamp, set)
	}
	if err != nil {
		return err
	}
...
func (t *Topic) replyGetData(sess *Session, asUid types.Uid, id string, incomingReqTs time.Time, req *MsgGetOpts) error {
	now := types.TimeNow()
	toriginal := t.original(asUid)
...
func (t *Topic) replyGetTags(sess *Session, asUid types.Uid, id string, incomingReqTs time.Time) error {
	now := types.TimeNow()
	...
	sess.queueOut(NoContentParams(id, t.original(asUid), now, incomingReqTs, map[string]string{"what": "tags"}))

	return nil
}

// replySetTags updates topic's tags - tokens used for discovery.
func (t *Topic) replySetTags(sess *Session, asUid types.Uid, incomingReqTs time.Time, set *MsgClientSet) error {
	var resp *ServerComMessage
	var err error
...
func (t *Topic) replyGetCreds(sess *Session, asUid types.Uid, id string, incomingReqTs time.Time) error {
	now := types.TimeNow()
	...
	sess.queueOut(NoContentParams(id, t.original(asUid), now, incomingReqTs, map[string]string{"what": "creds"}))

	return nil
}

// replySetCreds adds or validates user credentials such as email and phone numbers.
func (t *Topic) replySetCred(sess *Session, asUid types.Uid, incomingReqTs time.Time, authLevel auth.Level, set *MsgClientSet) error {
...
// a session as {meta}
// response goes to a single session rather than all sessions in a topic
func (t *Topic) replyGetDel(sess *Session, asUid types.Uid, id string, incomingReqTs time.Time, req *MsgGetOpts) error {
	now := types.TimeNow()
	toriginal := t.original(asUid)
...
func (t *Topic) replyDelMsg(sess *Session, asUid types.Uid, incomingReqTs time.Time, del *MsgClientDel) error {
	now := types.TimeNow()
...
func (t *Topic) replyDelTopic(h *Hub, sess *Session, asUid types.Uid, incomingReqTs time.Time, del *MsgClientDel) error {
	if t.owner != asUid {
		// Cases 2.1.1 and 2.2
		if t.cat != types.TopicCatP2P || t.subsCount() == 2 {
			return t.replyLeaveUnsub(h, sess, asUid, del.Id, incomingReqTs)
		}
	}
...
// Delete credential
func (t *Topic) replyDelCred(h *Hub, sess *Session, asUid types.Uid, incomingReqTs time.Time, authLvl auth.Level, del *MsgClientDel) error {
	now := types.TimeNow()
...
// Delete subscription
func (t *Topic) replyDelSub(h *Hub, sess *Session, asUid types.Uid, incomingReqTs time.Time, del *MsgClientDel) error {
	now := types.TimeNow()
...
func (t *Topic) replyLeaveUnsub(h *Hub, sess *Session, asUid types.Uid, id string, incomingReqTs time.Time) error {
	now := types.TimeNow()
	...
	if err := store.Subs.Delete(t.name, asUid); err != nil {
		if err == types.ErrNotFound {
			if id != "" {
				sess.queueOut(InfoNoAction(id, t.original(asUid), now, incomingReqTs))
			}
			err = nil
		} else if id != "" {
